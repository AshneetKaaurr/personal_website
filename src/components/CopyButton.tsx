...
/**
 * Copy-to-clipboard for the bios.
 *
 * Its own file so the pages that use it stay Server Components and can keep
 * exporting `metadata` — a page marked 'use client' cannot.
 */
export function CopyButton({ text, label }: { text: string; label: string }) {
  ...
  const handleCopy = async () => {
    try {
      ...
      // Clipboard access can be refused; leaving the label unchanged is the
